...
if not defined ADB_BIN (
  echo [quest3_usb_realstack_v5_08] adb.exe not found. Set ADB_EXE first.
  set "RC=2"
  goto :abort
)
if not exist "%ADB_BIN%" (
  echo [quest3_usb_realstack_v5_08] adb path does not exist: "%ADB_BIN%"
  set "RC=2"
  goto :abort
)

echo [quest3_usb_realstack_v5_08] adb=%ADB_BIN%
"%ADB_BIN%" devices
set "HAS_DEVICE="
...
if not defined HAS_DEVICE (
  echo [quest3_usb_realstack_v5_08] no adb device found.
  set "RC=3"
  goto :abort
)

echo [quest3_usb_realstack_v5_08] adb reverse tcp:%GATEWAY_PORT% ^> tcp:%GATEWAY_PORT%
...
if defined PORT_PID (
  echo [quest3_usb_realstack_v5_08] Port %GATEWAY_PORT% is already in use by PID %PORT_PID%.
...
if defined PORT_PID (
  echo [quest3_usb_realstack_v5_08] Port %INFERENCE_PORT% is already in use by PID %PORT_PID%.
  tasklist /fi "PID eq %PORT_PID%" 2>nul
  set "RC=5"
  goto :abort
)

rem ---- v5.08 defaults (can be overridden before launching) ----
...
if "%BYES_SERVICE_DET_PROVIDER%"=="yolo26" if "!HAS_ULTRALYTICS!"=="0" (
  echo [quest3_usb_realstack_v5_08] WARN: ultralytics missing, fallback det provider=mock
  set "BYES_SERVICE_DET_PROVIDER=mock"
)
if "%BYES_SERVICE_DEPTH_PROVIDER%"=="onnx" if "!HAS_ONNXRT!"=="0" (
  echo [quest3_usb_realstack_v5_08] WARN: onnxruntime missing, fallback depth provider=none
  set "BYES_SERVICE_DEPTH_PROVIDER=none"
)
...
echo [quest3_usb_realstack_v5_08] provider preflight:
...
if "%BYES_ENABLE_PYSLAM_REALTIME%"=="1" call :start_optional_service "%PYSLAM_PORT%" "BYES-pySLAM-v5.08" "services.pyslam_service.app:app"
if /I "%BYES_SERVICE_SEG_PROVIDER%"=="sam3" call :start_optional_service "%SAM3_PORT%" "BYES-SAM3-v5.08" "services.sam3_seg_service.app:app"
if /I "%BYES_SERVICE_DEPTH_PROVIDER%"=="da3" call :start_optional_service "%DA3_PORT%" "BYES-DA3-v5.08" "services.da3_depth_service.app:app"

echo [quest3_usb_realstack_v5_08] opening desktop console at http://127.0.0.1:%GATEWAY_PORT%/ui
start "" "http://127.0.0.1:%GATEWAY_PORT%/ui"

echo [quest3_usb_realstack_v5_08] Starting gateway + inference in this window. Press Ctrl+C to stop.
...
set "RC=%ERRORLEVEL%"
echo [quest3_usb_realstack_v5_08] stack exited with code %RC%
exit /b %RC%

:abort
if not defined RC set "RC=1"
echo [quest3_usb_realstack_v5_08] aborted with code %RC%
...
if not defined PORT_PID (
  echo [quest3_usb_realstack_v5_08] starting optional service %UVICORN_MODULE% on %PORT_TO_CHECK%...
  start "%WINDOW_TITLE%" cmd /c "cd /d \"%REPO_ROOT%\Gateway\" ^&^& python -m uvicorn %UVICORN_MODULE% --host 127.0.0.1 --port %PORT_TO_CHECK%"
) else (
  echo [quest3_usb_realstack_v5_08] service %UVICORN_MODULE% already listening on %PORT_TO_CHECK%, PID=%PORT_PID%.
)
exit /b 0
