...
def test_contract_suite_slam_context_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_slam_context_gate.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_slam_context_contract"), None)
    assert isinstance(row, dict)
    seg_lint = row.get("segLint", {})
    assert isinstance(seg_lint, dict)
    assert bool(seg_lint.get("slamContextPresent")) is True
    assert bool(seg_lint.get("slamContextSchemaOk")) is True
