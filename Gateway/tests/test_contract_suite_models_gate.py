...
def test_contract_suite_models_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_models_gate.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_models_manifest_contract"), None)
    assert isinstance(row, dict)
    seg_lint = row.get("segLint", {})
    assert isinstance(seg_lint, dict)
    assert bool(seg_lint.get("modelsPresent")) is True
    assert int(seg_lint.get("modelsMissingRequiredTotal", 1)) == 0
