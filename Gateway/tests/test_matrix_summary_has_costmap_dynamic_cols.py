...
def test_matrix_summary_has_costmap_dynamic_cols(tmp_path: Path) -> None:
    tests_dir = Path(__file__).resolve().parent
    gateway_dir = tests_dir.parent
    script = gateway_dir / "scripts" / "run_dataset_benchmark.py"

    src_pkg = tests_dir / "fixtures" / "run_package_with_costmap_dynamic_track_min"
    root = tmp_path / "root"
    pkg = root / "pkg_a"
    ...
    summary_md = (out_dir / "summary.md").read_text(encoding="utf-8-sig")
    assert "costmapDynTemporalUsedRate" in summary_md
    assert "costmapDynMaskUsedRate" in summary_md
    assert "costmapDynTracksUsedMean" in summary_md
