...
def test_contract_suite_slam_pose_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_slam_gate_test.json"
    ...
    slam_row = next((row for row in runs if str(row.get("id", "")) == "fixture_with_slam_pose_gt_contract"), None)
    assert isinstance(slam_row, dict)
    seg_lint = slam_row.get("segLint", {})
    assert isinstance(seg_lint, dict)
    assert bool(seg_lint.get("slamPoseEventsPresent")) is True
    assert bool(seg_lint.get("slamPosePayloadSchemaOk")) is True
