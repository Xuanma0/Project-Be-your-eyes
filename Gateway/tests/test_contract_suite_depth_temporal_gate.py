...
def test_contract_suite_depth_temporal_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_depth_temporal_gate_test.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_depth_temporal_contract"), None)
    ...
    assert bool(seg_lint.get("depthTemporalPresent")) is True
