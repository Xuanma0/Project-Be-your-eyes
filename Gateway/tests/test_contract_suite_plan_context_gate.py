...
def test_contract_suite_plan_context_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_plan_context_gate.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_plan_context_contract"), None)
