...
def _load_ocr_service_app() -> Any:
    root = Path(__file__).resolve().parents[1]
    module_path = root / "external" / "real_ocr_service" / "main.py"
    module_name = f"byes_real_ocr_service_{module_path.stat().st_mtime_ns}"
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError("failed to load real_ocr_service module")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    app = getattr(module, "app", None)
    if app is None:
        raise RuntimeError("real_ocr_service module has no FastAPI app")
    return app
...
def _create_tiny_onnx_model(path: Path) -> None:
    ...
    const_out = np.array([[[[0.5]]]], dtype=np.float32)

    nodes = [
        helper.make_node("Constant", inputs=[], outputs=["output0"], value=numpy_helper.from_array(const_out)),
    ]
    graph = helper.make_graph(nodes, "tiny_ocr_graph", [input_tensor], [output_tensor])
...
def test_real_ocr_service_healthz_ready(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pytest.importorskip("onnxruntime")
    model_id = "test-real-ocr-ready"
    ...
    app_missing = _load_ocr_service_app()
    with TestClient(app_missing) as client:
        ...
        infer_missing = client.post("/infer/ocr", files={"image": ("frame.jpg", _fake_image_bytes(), "image/jpeg")})
        assert infer_missing.status_code == 503

    _create_tiny_onnx_model(model_path)
    app_ready = _load_ocr_service_app()
    with TestClient(app_ready) as client:
        ...
        infer_ready = client.post("/infer/ocr", files={"image": ("frame.jpg", _fake_image_bytes(), "image/jpeg")})
