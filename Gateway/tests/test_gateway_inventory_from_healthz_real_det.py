...
def test_gateway_inventory_from_healthz_real_det(monkeypatch) -> None:
    original_enable_real_det = gateway.config.enable_real_det
    original_enabled_tools_csv = gateway.config.enabled_tools_csv
    original_enabled_tools = set(gateway._enabled_tools)  # noqa: SLF001

    object.__setattr__(gateway.config, "enable_real_det", True)
    object.__setattr__(gateway.config, "enabled_tools_csv", "mock_risk,mock_ocr,real_det")
    gateway._enabled_tools = gateway._parse_csv_tools(gateway.config.enabled_tools_csv)  # noqa: SLF001
    gateway.registry._tools.pop("real_det", None)  # noqa: SLF001

    async def _probe_ready(tool_name: str, endpoint: str) -> dict[str, Any]:
        if tool_name == "real_det":
            return {
                "tool": tool_name,
                "endpoint": endpoint,
    ...
    try:
        monkeypatch.setattr(gateway, "_probe_external_service", _probe_ready)
        with TestClient(app) as client:
            client.post("/api/dev/reset")
            assert "real_det" in _tool_names(client)

        monkeypatch.setattr(gateway, "_probe_external_service", _probe_not_ready)
        gateway.registry._tools.pop("real_det", None)  # noqa: SLF001
        with TestClient(app) as client:
            client.post("/api/dev/reset")
            assert "real_det" not in _tool_names(client)
            before = _parse_metrics(client.get("/metrics").text)
            _send_frames(client, 10)
            after = _wait_completed(client, before, expected_delta=10)

            unavailable_skip_delta = _metric_with_labels(
                after,
                "byes_tool_skipped_total",
                {"tool": "real_det", "reason": "unavailable"},
            ) - _metric_with_labels(
                before,
                "byes_tool_skipped_total",
                {"tool": "real_det", "reason": "unavailable"},
            )
            invoked_delta = _metric_with_labels(
                after,
            ...
            assert invoked_delta == 0
    finally:
        gateway.registry._tools.pop("real_det", None)  # noqa: SLF001
        object.__setattr__(gateway.config, "enable_real_det", original_enable_real_det)
