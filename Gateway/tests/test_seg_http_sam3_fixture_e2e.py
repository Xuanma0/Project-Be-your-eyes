...
def test_seg_http_sam3_fixture_service_e2e(tmp_path: Path) -> None:
    tests_dir = Path(__file__).resolve().parent
    gateway_root = tests_dir.parent
    fixture_src = tests_dir / "fixtures" / "run_package_with_sam3_fixture_seg_min"
    run_pkg = tmp_path / "run_pkg_seg_sam3_e2e"
    shutil.copytree(fixture_src, run_pkg)

    sam3_port = _pick_port()
    inf_port = _pick_port()
    sam3_log = tmp_path / "sam3_seg_service.log"
    ...
    try:
        ...
        sam3_probe = httpx.post(
            ...
            json={"runId": "fixture-sam3-seg", "frameSeq": 1, "image_b64": ""},
            timeout=3.0,
        )
        ...
        assert isinstance(sam3_segments[0].get("mask"), dict)
        ...
        with TestClient(app) as client:
            ...
            for frame_seq in (1, 2):
                ...
                meta = json.dumps({"runId": "fixture-sam3-seg", "sessionId": "fixture-sam3-seg", "frameSeq": frame_seq})
        ...
        report_md = tmp_path / "report_seg_http_sam3_e2e.md"
        report_json = tmp_path / "report_seg_http_sam3_e2e.json"
