...
def _upload(client: TestClient, tag: str) -> tuple[str, Path]:
    ...
    response = client.post(
        ...
        files={"file": ("plan_ack_haptic_cols.zip", payload, "application/zip")},
...
def _set_plan_ack_metrics(report_path: Path, *, haptic_rate: float) -> None:
    ...
    payload["planAck"] = {
        ...
        "ttsAckRate": 0.5,
        "arAckRate": 0.25,
        "hapticAckRate": float(haptic_rate),
        "byKindCounts": {"tts": 2, "ar": 1, "haptic": 1},
    }
    report_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def test_run_packages_leaderboard_has_plan_ack_haptic_col() -> None:
    with TestClient(app) as client:
        run_a, dir_a = _upload(client, "plan_ack_haptic_cols_a")
        run_b, dir_b = _upload(client, "plan_ack_haptic_cols_b")

        _set_plan_ack_metrics(dir_a / "report.json", haptic_rate=0.25)
        _set_plan_ack_metrics(dir_b / "report.json", haptic_rate=0.75)
        ...
        assert "haptic_ack_rate" in row_a
        assert "haptic_ack_rate" in row_b
        assert float(row_a.get("haptic_ack_rate", 0.0) or 0.0) != float(row_b.get("haptic_ack_rate", 0.0) or 0.0)
