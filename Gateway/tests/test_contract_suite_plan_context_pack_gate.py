...
def test_contract_suite_plan_context_pack_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_plan_context_pack_gate.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_plan_context_pack_contract"), None)
    assert isinstance(row, dict)
    seg_lint = row.get("segLint", {})
    assert isinstance(seg_lint, dict)
    assert bool(seg_lint.get("planContextPackEventsPresent")) is True
    assert bool(seg_lint.get("planContextPackSchemaOk")) is True
