...
def test_contract_suite_mode_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_mode_gate_test.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_mode_change_contract"), None)
    assert isinstance(row, dict)
    seg_lint = row.get("segLint", {})
    assert isinstance(seg_lint, dict)
    assert bool(seg_lint.get("modeEventsPresent")) is True
    assert bool(seg_lint.get("modeSchemaOk")) is True
    assert int(seg_lint.get("modeSwitches", 0) or 0) >= 2
