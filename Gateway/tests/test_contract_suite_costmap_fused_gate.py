...
def test_contract_suite_costmap_fused_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_costmap_fused_gate_test.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_costmap_fused_contract"), None)
