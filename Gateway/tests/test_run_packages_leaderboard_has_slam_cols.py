...
def _set_slam_quality(report_path: Path, *, tracking_rate: float, lost_rate: float, relocalized: int, p90: int) -> None:
...
def test_run_packages_leaderboard_has_slam_cols() -> None:
    with TestClient(app) as client:
        better_run_id, better_dir = _upload(client, "run_package_min", "slam_better_case")
        worse_run_id, worse_dir = _upload(client, "run_package_min", "slam_worse_case")

        _set_slam_quality((better_dir / "report.json"), tracking_rate=0.9, lost_rate=0.1, relocalized=1, p90=40)
        _set_slam_quality((worse_dir / "report.json"), tracking_rate=0.4, lost_rate=0.6, relocalized=0, p90=140)

        resp = client.get("/api/run_packages", params={"sort": "slam_tracking_rate", "order": "desc", "limit": 100})
        ...
        assert float(better.get("slam_tracking_rate", 0.0)) == 0.9
        assert float(worse.get("slam_tracking_rate", 0.0)) == 0.4
        assert items.index(better) < items.index(worse)

        filtered = client.get(
            "/api/run_packages",
            params={"min_slam_tracking_rate": 0.8, "max_slam_lost_rate": 0.2, "max_slam_latency_p90": 60, "limit": 100},
        )
        ...
        page = client.get("/runs", params={"sort": "slam_tracking_rate", "order": "desc"})
        assert page.status_code == 200, page.text
        assert "SLAM Tracking" in page.text
        assert "SLAM Lost" in page.text
        assert "SLAM p90(ms)" in page.text
