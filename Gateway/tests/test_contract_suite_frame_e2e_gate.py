...
def test_contract_suite_frame_e2e_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_frame_e2e_gate.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_frame_e2e_contract"), None)
    assert isinstance(row, dict)
    seg_lint = row.get("segLint", {})
    assert isinstance(seg_lint, dict)
    assert bool(seg_lint.get("frameE2eEventsPresent")) is True
    assert bool(seg_lint.get("frameE2eSchemaOk")) is True
