...
def test_dataset_benchmark_matrix_includes_costmap_columns(tmp_path: Path) -> None:
    tests_dir = Path(__file__).resolve().parent
    gateway_dir = tests_dir.parent
    script = gateway_dir / "scripts" / "run_dataset_benchmark.py"

    src_pkg = tests_dir / "fixtures" / "run_package_with_costmap_min"
    root = tmp_path / "root"
    pkg = root / "pkg_a"
    ...
    assert "costmapCoverage(mean)" in text
    assert "costmapLatencyP90(p90)" in text
    assert "planCostmapUsedRate(mean)" in text

    latest_csv = out_dir / "latest.csv"
    header = latest_csv.read_text(encoding="utf-8-sig").splitlines()[0]
    assert "costmap_coverage" in header
    assert "costmap_latency_p90" in header
    assert "plan_costmap_ctx_used_rate" in header
