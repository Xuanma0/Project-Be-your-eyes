...
def test_contract_suite_seg_tracking_gate_passes(tmp_path: Path) -> None:
    ...
    out_path = tmp_path / "contract_seg_tracking_gate_test.json"
    ...
    row = next((item for item in runs if str(item.get("id", "")) == "fixture_with_sam3_tracking_contract"), None)
    ...
    assert bool(seg_lint.get("segTrackPresent")) is True
    assert bool(seg_lint.get("segTrackSchemaOk")) is True
