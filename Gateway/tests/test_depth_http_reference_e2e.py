...
def test_depth_http_reference_e2e(tmp_path: Path) -> None:
    tests_dir = Path(__file__).resolve().parent
    gateway_root = tests_dir.parent
    fixture_src = tests_dir / "fixtures" / "run_package_with_depth_gt_min"
    run_pkg = tmp_path / "run_pkg_depth_e2e"
    shutil.copytree(fixture_src, run_pkg)

    ref_port = _pick_port()
    inf_port = _pick_port()
    ref_log = tmp_path / "reference_depth_service.log"
    ...
    try:
        ...
        ref_env["BYES_REF_DEPTH_FIXTURE_DIR"] = str(run_pkg)
        ref_env["BYES_REF_DEPTH_RUN_ID"] = "fixture-depth-gt"
        ref_proc, ref_log_file = _start_uvicorn(
            module_path="services.reference_depth_service.app:app",
        ...
        ref_probe = httpx.post(
            f"http://127.0.0.1:{ref_port}/depth",
            json={"runId": "fixture-depth-gt", "frameSeq": 1, "image_b64": ""},
            timeout=3.0,
        )
        assert ref_probe.status_code == 200, ref_probe.text
        ref_payload = ref_probe.json()
        jsonschema.validate(ref_payload, schema)
        assert isinstance(ref_payload.get("grid"), dict)

        inf_env = dict(os.environ)
        inf_env["BYES_SERVICE_DEPTH_PROVIDER"] = "http"
        inf_env["BYES_SERVICE_DEPTH_ENDPOINT"] = f"http://127.0.0.1:{ref_port}/depth"
        inf_env["BYES_SERVICE_DEPTH_MODEL_ID"] = "reference-depth-v1"
        ...
        with TestClient(app) as client:
            ...
            for frame_seq in (1, 2):
                ...
                meta = json.dumps({"runId": "fixture-depth-gt", "sessionId": "fixture-depth-gt", "frameSeq": frame_seq})
        ...
        report_md = tmp_path / "report_depth_http_e2e.md"
        report_json = tmp_path / "report_depth_http_e2e.json"
