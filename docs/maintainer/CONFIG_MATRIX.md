...
| `BYES_ASSET_CACHE_TTL_MS` | TTL for `/api/assets/*` cache entries used by `seg.mask.v1` / `depth.map.v1` events; `v5.08.2` realstack raises this to `30000` to avoid premature overlay expiry during hold rendering | `12000` repo default, `30000` in `quest3_usb_realstack_v5_08_2.cmd` | Gateway asset cache | `Gateway/main.py:739`; `Gateway/byes/asset_cache.py`; `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
...
| `BYES_PYSLAM_ROOT` | Optional local pySLAM root used by the `v5.08.2` Quest USB realstack preflight; missing or invalid root is reported as `UNAVAILABLE_MISSING_PATH` and pySLAM stays unavailable | empty | Quest USB launcher | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_PROVIDER_DET` | `v5.08.2` realstack alias for DET provider profile (`yolo26|ultralytics|mock`); launcher prefers this alias unless shell env explicitly sets `BYES_SERVICE_DET_PROVIDER` | `yolo26` | Quest USB launcher | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_PROVIDER_SEG` | `v5.08.2` realstack alias for SEG provider profile (`sam3|mock`); launcher prefers this alias unless shell env explicitly sets `BYES_SERVICE_SEG_PROVIDER` | `sam3` | Quest USB launcher | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_PROVIDER_DEPTH` | `v5.08.2` realstack alias for DEPTH provider profile (`da3|onnx|none|mock`); launcher prefers this alias unless shell env explicitly sets `BYES_SERVICE_DEPTH_PROVIDER` | `da3` | Quest USB launcher | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_YOLO26_WEIGHTS` | YOLO26 local weights path required for DET `READY_REAL` in the `v5.08.2` realstack preflight; missing path yields `UNAVAILABLE_MISSING_PATH` | empty | Quest USB launcher / inference det | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_DA3_WEIGHTS` | Optional DA3 depth alias path mapped to `BYES_DA3_MODEL_PATH` / `BYES_SERVICE_DEPTH_MODEL_PATH`; missing path yields `UNAVAILABLE_MISSING_PATH` when depth provider is `da3` | empty | Quest USB launcher / inference depth | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_SAM3_WEIGHTS` | Optional SAM3 checkpoint alias mapped to `BYES_SAM3_CKPT`; missing path yields `UNAVAILABLE_MISSING_PATH` when seg provider is `sam3` | empty | Quest USB launcher / inference seg | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_SAM3_CKPT` | Preferred SAM3 checkpoint path for `v5.08.2` realstack fail-closed preflight | empty | Quest USB launcher / inference seg | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
| `BYES_DA3_MODEL_PATH` | Preferred DA3 model path for `v5.08.2` realstack fail-closed preflight | empty | Quest USB launcher / inference depth | `tools/quest3/quest3_usb_realstack_v5_08_2.cmd` |
